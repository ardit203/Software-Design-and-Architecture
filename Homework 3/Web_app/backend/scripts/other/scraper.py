...
def fetch_prediction_issuers():
    ...
    pd.DataFrame(codes, columns=['Code']).to_csv(f'{FOLDER}/Issuers-Prediction.csv', index=False)
...
def fetch_issuers():
    ...
    for option in issuers_dropdown.find_all('option'):
        ...
        if code and not any(char.isdigit() for char in code):
            issuers.append(code)
    pd.DataFrame(issuers, columns=['Code']).to_csv(f'{FOLDER}/Issuers.csv', index=False)
    return issuers
...
def fetch_range(companies, code, start_date, end_date):
    ...
    if stock_data:
        ...
        res.to_csv(f'{FOLDER}/{code}.csv', index=False)
...
def fetch_data(companies, issuers):
    ...
    for code in issuers:
        ...
        if f'{code}.csv' not in os.listdir(f'{FOLDER}'):
            search_from = range_in_days(today, 365 * 10)
        else:
            issuer_df = pd.read_csv(f'{FOLDER}/{code}.csv')
    ...
    for code in issuers:
        ...
        if code in companies.keys():
            ...
            df.to_csv(f'{FOLDER}/{code}.csv', index=False)
...
if __name__ == '__main__':
    print(fetch_prediction_issuers())
#     start = time.time()
#     issuers = fetch_issuers()
#     companies = dict()
#     fetch_data(companies, issuers)
