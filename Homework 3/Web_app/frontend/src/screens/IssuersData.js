...
const IssuersData = () => {
  ...
  const fetchNewData = () => {
      axios
        .post("http://127.0.0.1:5000/api/fetch-new-data")
        .then((response) => {
          console.log("New data fetched:", response.data.message); // Debugging: Log success message
  ...
  const fetchIssuerData = () => {
    ...
    axios
      .post("http://127.0.0.1:5000/api/issuer-data", {
    ...
    <div>
      ...
      <div className="issuers-data-2">
        <div className="table-info-2">
          ...
          <div className="find-2">
            <button className="btn-2" onClick={fetchNewData}>
              Fetch New Data
            </button>
          </div>
        </div>
          ...
          {chartData ? (
            <Line
              ...
              options={{
                ...
                plugins: {
                  legend: { position: "top" },
                  title: { display: true, text: `Feature: ${selectedFeature}` },
                  zoom: {
